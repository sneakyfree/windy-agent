...
- **Owner dashboard** at `https://agent.windyword.ai` — sign-in is "Sign in with
...
1. Prerequisites:
   ...
   - The hub lane has registered the public OAuth client (`HUB_OAUTH_CLIENT_ID`) with the loopback and `https://agent.windyword.ai/api/auth/hub/callback` redirects.
...
4. Route it:
   `sudo cloudflared tunnel route dns e2f8bb11-060e-4707-8c57-6852b4c6d24d agent.windyword.ai`,
   add the snippet above the 404 rule in `/etc/cloudflared/config.yml`, run
   `sudo systemctl restart cloudflared`, then `curl -s https://agent.windyword.ai/api/health`.
5. Tell the hub lane the webhook URL: `https://agent.windyword.ai/api/webhooks/trust`.
6. Tell lane 80 to add `https://agent.windyword.ai/api/health` to the uptime checks.
