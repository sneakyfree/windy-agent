...
#: free model for a free-tier caller (its free-auto routing, live since
#: 2026-07-04). If Mind is unreachable there's no paid provider to fall
#: to — the agent drops to the Ollama lifeboat, the correct degradation
#: for a no-key agent.
KEYLESS_MODEL = "windy-mind-auto"


def write_keyless_config(preset: str = "buddy") -> None:
    """Write .env + windyfly.toml for a KEYLESS agent (Sprint 5).

    No API key. The brain is Windy Mind's free-compute lane, reached
    with the Eternitas Passport Token that hatch mints + persists (PR
    #247). Tools are enabled because Mind's tool-calling shipped
    ...
    env_lines = [
        "# Windy Fly — generated by `windy go` (keyless / Windy Mind brain)",
        f"DEFAULT_MODEL={KEYLESS_MODEL}",
        "",
...
def is_keyless_configured() -> bool:
    """True if a keyless (Windy Mind) config is already written."""
    env_file = PROJECT_ROOT / ".env"
...
def _go_keyless(args: Any) -> None:
    """The zero-key grandma path: free agent, no account, no key.

    Writes the keyless config, runs the hatch ceremony (which mints +
    persists the Eternitas passport the brain authenticates with), then
...
def cmd_go(args: Any) -> None:
    ...
    if getattr(args, "keyless", False):
        # Scripted/ballroom kiosk path: free Windy Mind agent, no prompts.
        _go_keyless(args)
        return
    ...
    # ── Step 5: Ask how they want a brain ────────────────────────
    # The keyless free path is option 1 (the default) — that's the
    # grandma/ballroom flow: a working agent with no key, no account.
    console.print("  [bold]How do you want to power your agent?[/bold]")
    console.print()
    console.print(
        "    [bold]1[/bold]  [green]Free — no key needed[/green] "
        "[dim](Windy Mind free compute; recommended)[/dim]"
    )
    console.print("    [dim]Or bring your own AI provider key:[/dim]")
    for i, p in enumerate(PROVIDER_MENU, 2):
    ...
    if idx == 1:
        # The keyless grandma path: free agent via Windy Mind.
        _go_keyless(args)
        return

    # Options 2..N+1 map to PROVIDER_MENU[0..N-1] (BYO-key path).
    provider_idx = idx - 2
    if provider_idx < 0 or provider_idx >= len(PROVIDER_MENU):
        provider_idx = 0

    selected = PROVIDER_MENU[provider_idx]
    console.print()
