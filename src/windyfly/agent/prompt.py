...
def assemble_prompt(
    ...
    *,
    ...
    # Phase 2.3.2 (partial — 2nd section extracted) — content now in
    # prompt_sections/runtime_guardrail.py, byte-identical.
    from windyfly.agent.prompt_sections import RUNTIME_GUARDRAIL_TEXT
    system_parts.append(RUNTIME_GUARDRAIL_TEXT)
    ...
    # Phase 2.3.2 (partial) — section moved to
    # agent/prompt_sections/bias_to_action.py. Byte-identical content
    # via BIAS_TO_ACTION_TEXT constant; this is the FIRST split per
    # the proposed layout in docs/PROMPT_AS_BUILT.md §6. Future
    # sections migrate the same way — define constant in
    # prompt_sections/<name>.py, import + append here. Full
    # render(ctx)-based composition (with PromptContext + conditional
    # gating moving into each section) waits on Grant's design Qs.
    from windyfly.agent.prompt_sections import BIAS_TO_ACTION_TEXT
    system_parts.append(BIAS_TO_ACTION_TEXT)
    ...
    if band is not None:
        ...
        if band_value is not None and band_value < 2:
            from windyfly.agent.prompt_sections import GRANDMA_MODE_TEXT
            system_parts.append(GRANDMA_MODE_TEXT)
