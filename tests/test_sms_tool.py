...
class TestBootSequenceWiring:
    def test_default_sequence_includes_tools_sms(self) -> None:
        ...
        # SMS registers right after chat. The slot AFTER sms used to be
        # cloud; D.3.2 inserted voice between sms and cloud (sms+voice are
        # both windy-call-family channels — paired in the registry).
        sms_idx = names.index("tools.sms")
        assert names[sms_idx - 1] == "tools.chat"
        assert names[sms_idx + 1] in ("tools.cloud", "tools.voice")
